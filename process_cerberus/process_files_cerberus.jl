...
function promofull_file_to_dataframe(file_path::String, decompressor_object::Decompressor)::DataFrame
    ...
    for item_node in item_nodes
        ...
        for element in eachelement(item_node)
            if element.name in ["PromotionId", "AllowMultipleDiscounts", "RewardType", "MinNoOfItemOfered", "AdditionalIsCoupon", "AdditionalGiftCount", "AdditionalIsTotal", "AdditionalIsActive", "DiscountRate","DiscountType","WeightUnit"]
            ...
            elseif element.name == "PromotionItems"
                ...
                # NOTE: some stores dont have the count in the tag, but apparently getting the amount of child nodes is fairly fast
                resize!(promotion_items, EzXML.countelements(element))
    ...
    for item_node in item_nodes
        ...
        for element in eachelement(item_node)
            if element.name in ["ItemCode"]
                row[element.name] = parse(Int64, element.content)
            elseif element.name in ["Quantity", "ItemPrice", "UnitOfMeasurePrice", "ItemId"]
                row[element.name] = parse(Float64, element.content)
            elseif element.name in ["PriceUpdateDate", "ItemType", "ItemName", "QtyInPackage", "ItemNm", "ManufacturerName", "ManufactureCountry", "ManufacturerItemDescription", "UnitQty", "bIsWeighted", "UnitOfMeasure", "AllowDiscount", "ItemStatus"]
                row[element.name] = element.content
            else
...
function processFiles(STORE_NAME::String, CERBERUS_PASSWORD::String, GLOBAL_FOLDER::String)
